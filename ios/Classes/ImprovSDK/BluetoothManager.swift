...
final class BluetoothManager: NSObject, BluetoothManagerProtocol {
    ...
    private let rpcResultParser = RpcResultParser()
...
extension BluetoothManager: @preconcurrency CBPeripheralDelegate {
    ...
    public func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if let value = characteristic.value {
            ...
            case BluetoothUUIDs.charRpcResult:
                ...
                for result in extractResultStrings(from: value) {
    ...
    private func extractResultStrings(from data: Data) -> [[String]] {
        let results = rpcResultParser.append(data)
        if results.isEmpty, !data.isEmpty {
            Logger.main.debug("RPC Result fragment buffered or dropped")
        }
        return results
    }
